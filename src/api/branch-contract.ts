...
export const branchesContract = oc.tag(OPEN_API_TAGS.branches).router({
  list: oc.route({ method: 'GET', path: '/branches', summary: 'List branches' }).output(z.object({ branches: z.array(branchSchema) })),
  retrieve: oc
  ...
  reset: oc
    .route({
      method: 'POST',
      path: '/branches/{slug}/reset',
      summary: 'Reset branch from parent',
    })
    .input(branchSlugInput)
    .output(
      z.object({
  ...
  expiry: {
    update: oc
      .route({
        method: 'PATCH',
        path: '/branches/{slug}/expiry',
        summary: 'Update branch expiry',
      })
